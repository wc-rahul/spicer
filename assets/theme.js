...
theme.Cart = (function () {
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
            ...
            if (shippingRates.length === 0) {
              ...
              shippingRates.forEach(function (item) {
                html +=
                  ...
                  theme.Currency.formatMoney(item["price"], theme.moneyFormat) +
                  "</span></li>";
              });
    ...
    _rerenderCart: function () {
      ...
      return fetch(
        ...
        content.text().then(function (html) {
          ...
          var CartSpecialInstructionsSubmit = _this.container.querySelector(
            ".CartSpecialInstructionsSubmit"
          );
          if (CartSpecialInstructionsSubmit) {
    ...
    _replaceContent: function (html) {
      ...
      var drawer__CartCount = document.querySelector(".drawer_cart_count");
      var header_total_priceEl = document.querySelector(".cart_total_price");
      var drawer_header = document.querySelector("[data-cart-heading]");
      var drawer_header_count = document.querySelector(
      ...
      if (drawer__CartCount) {
        drawer__CartCount.innerHTML = this.itemCount;
      }
      if (header_total_priceEl) {
        header_total_priceEl.innerHTML = theme.Currency.formatMoney(
          this.totalPrice,
          theme.moneyFormat
        );
      }
      if (this.itemCount > 0) {
        drawer_header.innerHTML = drawer_header_count.innerHTML;
      } else {
        drawer_header.innerHTML = "Your Cart is Empty";
      }
      //
    },
  ...
  function AnnouncementBar(container) {
    ...
    if (this.AnnouncementBar__Slider) {
      ...
      function slideHeightCalculate() {
        ...
        var slidesLength = _this.AnnouncementBar__Slider.querySelectorAll(
        ...
        if (slidesLength > 1) {
          ...
          setTimeout(function () {
            document.documentElement.style.setProperty(
              ...
              document.querySelector('[data-section-type="announcement-bar"]')
                .offsetHeight + "px"
            );
          }, 200);
        }
   ...
   // theme.ProductItemSwatches();
    ...
    if (filter_titles.length) {
      filter_titles.forEach(function (item, index) {
        ...
        var filtersBlock = parentArea.querySelector(
    ...
    var close_sidebar_btn = container.querySelector(
    ...
    if (price_range_inputs.length) {
      document.addEventListener("change", function (e) {
        ...
        if (el.classList.contains("price_range")) {
          ...
          if (FitersFormEl) {
            var prices_arr = [];
            var price_range_inputs = FitersFormEl.querySelectorAll(
              ".filter-group-display__price-range-input"
            );
  ...
  function AdvancedSearch(container) {
    ...
    if (submit_btn) {
      submit_btn.addEventListener("click", function (e) {
        ...
        var formValue = form.querySelector(".search-form__input").value;
        var formProductType = form.querySelector(
        ...
        var formAction =
          window.routes.searchUrl + "?q=" + productQuery + "&type=product";
        debugger;
        window.location.href = formAction;
      });
    }
...
theme.CountdownTimer = (function () {
  function CountdownTimer(container) {
    ...
    if (countdown_timer) {
      ...
      var timer_behaviour = countdown_timer.getAttribute(
        "data-timer_behaviour"
      );
      var countDownDate = new Date(dateStr).getTime();

      var timerDaysElem = countdown_timer.querySelector(
        ".timer-days .timer-value"
      );
      var timerHoursElem = countdown_timer.querySelector(
        ".timer-hours .timer-value"
      );
      var timerMinutesElem = countdown_timer.querySelector(
        ".timer-minutes .timer-value"
      );
      var timerSecondsElem = countdown_timer.querySelector(
...
theme.FeaturedProducts = (function () {
  function FeaturedProducts(container) {
    ...
    function updateFlickityConfig(sliderEl) {
      ...
      if (windowWidth < 1200) {
        slider_options.groupCells = 1;
      } else if (slidesCount < 4) {
        slider_options.groupCells = slidesCount;
      } else {
        slider_options.groupCells = 4;
      }
      return slider_options;
    }

    function toggleArrowsVisibility(sliderEl, slider_custom_arrows) {
      var slidesCount = sliderEl.querySelectorAll(".product-item-block").length;
      ...
      if (slidesCount <= groupCells) {
        ...
        slider_custom_arrows.classList.remove("hide"); // Show arrows otherwise
      }
    }

    if (sliderEl) {
      slider_options = updateFlickityConfig(sliderEl); // Get updated options based on screen size
      ...
      if (slider_custom_arrows) {
        ...
        toggleArrowsVisibility(sliderEl, slider_custom_arrows);

        var slider_custom_btns = slider_custom_arrows.querySelectorAll(".slider-btn");
        slider_custom_btns.forEach(function (item) {
          item.addEventListener("click", function (e) {
            e.preventDefault();
            var isNext = item.classList.contains("button-next");
            if (isNext) {
    ...
    if (tabs_nav) {
      tabs_nav.querySelectorAll(".index-tabs_nav--item").forEach(function (item) {
        item.addEventListener("click", function (e) {
          e.preventDefault();
          ...
          if (content_block) {
            ...
            if (tab_content_slider) {
              slider_options = updateFlickityConfig(tab_content_slider); // Update options based on the new tab
              ...
              toggleArrowsVisibility(tab_content_slider, slider_custom_arrows);
            }
          }
        });
      });
...
theme.Product = (function () {
  function Product(container) {
    ...
    this.cartHeading = container.querySelector(this.selectors.cartHeading);

    var send_gift_card_to_recipient = container.querySelector(
      "#send_gift_card_to_recipient"
    );
    if (send_gift_card_to_recipient) {
      send_gift_card_to_recipient.addEventListener("change", function (e) {
        var send_gift_card_to_recipient_fields_wrapper = this.closest(
          ".gift-card-recipient"
        ).querySelector(".gift-card-recipient__fields");
        var send_gift_card_to_recipient_field_email =
          send_gift_card_to_recipient_fields_wrapper.querySelector(
            ".gift-card-recipient-field-email"
          );
        if (send_gift_card_to_recipient_fields_wrapper) {
          if (this.checked) {
            send_gift_card_to_recipient_fields_wrapper.style.maxHeight =
              send_gift_card_to_recipient_fields_wrapper.scrollHeight + "px";
            send_gift_card_to_recipient_field_email.setAttribute(
              "required",
              ""
            );
          } else {
      ...
      product_description_accrordion.forEach(function (accordionEl, index) {
        ...
        if (product_accordion__blocks.length) {
          product_accordion__blocks.forEach(function (accordionBlock, index) {
            ...
            var content = accordionBlock.querySelector(
      ...
      Shopify.updateOptionsInSelector = function (selectorIndex) {
        switch (selectorIndex) {
          ...
          case 2:
            ...
            key +=
              " / " +
              parentForm.querySelectorAll(".single-option-selector")[1].value;
            selector = parentForm.querySelectorAll(
              ".single-option-selector"
            )[2];
        }
        ...
        if (swatchElements.length) {
          ...
          if (inputElements.length == 0) {
            var swatchFirstElements = Array.from(
              parentForm.querySelectorAll(
                '.swatch[data-option-index="' +
                  selectorIndex +
              ...
              allSelectOptionsEl.forEach(function (item, index) {
                ...
                if (itemText == variant.title) {
                  ...
                  var swatchEl = parentForm.querySelector(
                    ".product_swatches [data-value='" + variant.title + "']"
                  );
      ...
      if (back_in_stock_button) {
        back_in_stock_button.addEventListener("click", function (e) {
          ...
          if (back_in_stock_input) {
            ...
            var contactFormEl = document.querySelector("#" + contact_formID);
            if (contactFormEl) {
              contactFormEl.querySelector(".contact-form_email").value =
    ...
    var complementary_productsEl = container.querySelector(
      ".complementary-products"
    );
    if (complementary_productsEl) {
      var recommendationsSectionUrl =
        complementary_productsEl.getAttribute("data-url");

      window.performance.mark(
        "pursuit:product:fetch_product_recommendations.start"
      );

      var complementary_products_wrapperEl =
        complementary_productsEl.querySelector(
      ...
      fetch(recommendationsSectionUrl)
        ...
        .then(function (productHtml) {
          ...
          var new_complementary_products_wrapperEl = tempDiv.querySelector(
            ".complementary-products-slider"
          );
          var new_complementary_products_arr =
            new_complementary_products_wrapperEl.querySelectorAll(
              ".complementary_products_slide"
            );

          if (
            complementary_products_wrapperEl &&
            new_complementary_products_arr.length
          ) {
            complementary_products_wrapperEl.innerHTML =
              new_complementary_products_wrapperEl.innerHTML;
            complementary_products_wrapperEl.innerHTML =
              complementary_products_wrapperEl.firstElementChild.innerHTML;

            var add_Complementary_products =
              complementary_productsEl.querySelectorAll(
              ...
              add_Complementary_products.forEach(function (btn, index) {
                btn.addEventListener("click", function (e) {
                  ...
                  fetch("/cart/add.js", {
                    ...
                    .then(function (json) {
                      var addToCartBtnText = btn.querySelector(
                        "[data-add-to-cart-text]"
                      );
                      ...
                      setTimeout(function () {
                        ...
                        if (self.show_preOrder_btn) {
                          var isPreOrderBtn = btn.hasAttribute(
                            "data-preOrderButton"
                          );
                          if (isPreOrderBtn) {
                            addToCartBtnText.innerHTML = theme.strings.preOrder;
            ...
            let slider_options = JSON.parse(
              complementary_products_wrapperEl.getAttribute(
            ...
            var slider_custom_arrows = complementary_productsEl.querySelector(
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _initVariants: function () {
      ...
      if (this.pickAnOption) {
        ...
        if (variantsSize > 1) {
          ...
          var submitFormButton = this.productForm.querySelector(
    ...
    _updateInventoryQuantity: function (evt) {
      ...
      if (inventory_qty_info) {
        ...
        var inventory_icon_statusEl = this.container.querySelector(
        ...
        this.swatchesWrapper
          ...
          .forEach(function (item, index) {
            item.addEventListener("change", function (e) {
              ...
              var triggerSelectEl = this.closest("form").querySelectorAll(
                ".single-option-selector"
              )[optionIndex];
              if (triggerSelectEl) {
