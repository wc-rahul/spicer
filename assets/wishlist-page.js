...
class WishlistProductCard extends WishlistElement {
  ...
  getEventConfig() {
    return {
      ...
      "submit .wk-form": async (event) => {
        ...
        var dataElement = document.querySelector('#HeaderCart');
        var link_type = dataElement.getAttribute('data-link-type');
        if( link_type == 'drawer') {
          theme.Cart.prototype._onProductAdded().then(function () {
  ...
  renderPrice({ product, variant }) {
    if (!this.app.customer || !this.showPrice) {
      return;
    }
    return html`
      <div class="wk-price">
  ...
  //       //  return html`
  //       //   <a
  //       //     class="wk-cta-button" href=${this.app.routes.accountLoginUrl} >
  //       //     <span class="wk-cta-label">${getCtaText()}</span>
  //       //   </a>
  //       // `;
  }
